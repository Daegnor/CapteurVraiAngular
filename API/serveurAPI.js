...
const requeteInsert = "INSERT INTO DATA(LIB_DATA, VAL_DATA, DTE_DATA, CLE_SENSOR) VALUES(@Lib, @Val, DEFAULT, (SELECT CLE_SENSOR from SENSOR where LIB_SENSOR LIKE @Capteur));";

const requeteGet = "SELECT LIB_DATA, VAL_DATA FROM DATA WHERE CLE_DATA IN (SELECT MAX(CLE_DATA) from DATA GROUP BY CLE_SENSOR, LIB_DATA HAVING CLE_SENSOR = (SELECT CLE_SENSOR from SENSOR where LIB_SENSOR LIKE @Capteur));";

const requeteGraph = "SELECT VAL_DATA, DTE_DATA FROM DATA WHERE DTE_DATA >= @Yesterday and DTE_DATA < @Today and LIB_DATA LIKE @Lib and CLE_SENSOR = (SELECT CLE_SENSOR from SENSOR where LIB_SENSOR LIKE @Capteur) ORDER BY DTE_DATA ASC";
